...
### 主限法(承前)
- Placidus 方位法(`perpredict.getPrimaryDirectionByZPlacidus` + `_PD_METHOD_REGISTRY`,不动 `_byZCoreKernel`)。
- **time-key 方法论纠正**:只保留公式可证的 `Ptolemy / Naibod`(撤除 Cardano/Plantiko/Wöllner/SymbolicSolarArc 拟合值 + 删 `scripts/fit_pd_constants.py`)。前端 time-key 下拉同步只剩 2 个。
...
- 真栈:七政四余切「回归今制」七政落宿应与参考一致(日翼/火轸/木氐…),命度红线在宿度带可见;主限法 time-key 只剩 Ptolemy/Naibod。

### 本批新增
- **新增方位法 `Placidus`**(`pdMethod = 'placidus'`)—— 现代主流主限法,与 Placidus 宫位制天然配套。后端在 `perpredict.getPrimaryDirectionByZPlacidus` 落地(复用 flatlib PrimaryDirections 默认 Placidian semi-arc 算法,不带 Alcabitius 专属 ML 修正层)。
- **新增 5 个静态时间换算 + 1 个符号时间换算**(`pdTimeKey ∈ {Cardano, Plantiko, Wollner, SymbolicDegree, SymbolicSolarArc}` + 既有 Ptolemy/Naibod) —— 缩放常量在 `perpredict.STATIC_TIME_KEY_SCALES` 表中(基于内部校准语料拟合得出,Ptolemy 锁 1.0 守字节级一致,其它 6 个 P0 阶段以拟合常量上线)。
...
- **前端选项扩** —— `AstroPrimaryDirection.js`(表格) + `AstroPrimaryDirectionChart.js`(盘) 两 Select Option 数组各扩到 3 方位法 + 7 timeKey。下拉宽度用 `dropdownMatchSelectWidth={false}` 防长 label 截断。label 字典集中到 `primaryDirectionSync.PD_METHOD_LABELS/PD_TIME_KEY_LABELS` + `getPdMethodLabel/getPdTimeKeyLabel` helper。
...
- **新增脚本**:`Horosa-Web/astropy/scripts/fit_pd_constants.py`(自研拟合脚本,基于内部校准语料反推 static time-key 常量)。
...
- preflight `[32]` 阻断「Alcabitius byteperfect 失效 / Ptolemy != 1.0 / _byZCoreKernel 改名」;`[33]` 阻断「placidus 白名单缺 / PD_SYNC_REV 未升 / Naibod 表格仍降级 / aiContext 仍硬编码」。
- 真栈手测:启动 desktop app,主限法表格 + 主限法盘下拉应当出现 Alchabitius/Placidus/Horosa原方法 + Ptolemy/Naibod/Cardano/Plantiko/Wollner/符号度/太阳弧符号;默认 Alchabitius+Ptolemy 表格逐行与 v2.5.3 完全一致;切到 Placidus + 各 timeKey 表格能渲染、盘能画。
